...
import static fr.uga.pddl4j.planners.Planner.DEFAULT_TIMEOUT;
...
public class HSPContext {
    private String error = "";
    private fr.uga.pddl4j.heuristics.relaxation.Heuristic.Type heuristic;
    ...
    public void setHSPContext(Heuristic.Type heuristic, double weight, double timeout){
        this.heuristic = heuristic;
        this.weight = weight;
        this.timeout = timeout;
    }

    public boolean resolveWithHSP(Token token) throws IOException {
        if (token.isRunnable()) {
            ...
            try {
                ...
                planner.setHeuristicType(heuristic);
                planner.setWeight(weight);
                planner.setTimeOut((int) timeout);
